...
from .network import TripletLoss, SetNet, BaseNet, RGNet, ROINet,GATNet,SF1Net
...
class Model:
	def __init__(self,
		...
		self.encoder = RGNet(self.hidden_dim).float()
	...
	def collate_fn(self, batch):
		...
		poses = [batch[i][5] for i in range(batch_size)] 		#增加pose
		batch = [seqs, poses, view, seq_type, label, None]	#增加pose
	...
	def fit(self):
		...
		_time1 = datetime.now()
		for seq, pose, view, seq_type, label, batch_frame in train_loader:
			#print(batch_frame)
			self.restore_iter += 1
